...
def linear_model(t, x0, F, k, omega):
    return (x0-45*F/k)*np.cos(omega*t)
...
angle = np.around(np.array(data['Degree'].ravel()), 2)[start_point:end_point] * np.pi / 180
[A_, omega_, phi_, zeta_] = curve_fit(exp_model, time, angle)[0]
exp_fit = exp_model(time, A_, omega_, phi_, zeta_)
[x0, F, k, omega] = curve_fit(linear_model, time, angle,[-1.57079633,0.0002222461477312496,0.0039422038081536,5.28])[0]
print("Force is:%f" % F)
print("Stiffness is:%f" % k)
