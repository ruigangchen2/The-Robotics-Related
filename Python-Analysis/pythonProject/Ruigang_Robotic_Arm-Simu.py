...
velocity = np.around(np.array(data['Velocity'].ravel()), 2)
...
def EOM(_t, y):  # y2 =  k/J * (90 + y0)
    y1,y2 = y
    return [y2, stiffness / interia_total * (y1 + 90)]

sol_ivp = solve_ivp(EOM, [0, 0.3], [initial_theta, initial_speed], max_step=0.001)
...
plt.subplot(212)
plt.plot(time, velocity_filter, 'b-*', label='Velocity [degrees/s]')
plt.plot(sol_ivp.t, sol_ivp.y[1], 'r-*', label="Velocity Simu [degrees/s]")
plt.xlabel('Time [s]', fontweight='bold')
plt.ylabel('Angular Velocity [degrees/s]', fontweight='bold')
plt.grid()
plt.legend()
plt.ylim([-300, 300])
plt.show()
