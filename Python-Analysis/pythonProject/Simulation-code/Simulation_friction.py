...
clutch2 = np.append([1, 1, 1, 1, 1], clutch2)
...
Electromagnet_1_Clutch = clutch1[5:-8]
Electromagnet_2_Clutch = clutch2[5:-8]
Electromagnet_3_Clutch = clutch3[5:-8]
Electromagnet_4_Clutch = clutch4[5:-8]
...
for i in range(len(time_EA)):
    if Electromagnet_2_Clutch[i] == 1:
        Upper_Elastic_Energy[i] = 0.5 * k1 * ((theta_fitted[i]) ** 2)
...
for i in range(len(time_EA)):
    if Electromagnet_3_Clutch[i] == 1:
        ...
        Lower_Elastic_Energy[i] = 0.5 * k2 * ((abs(theta_fitted[i]) - abs(theta_fitted[lower_matrix_start_number])) ** 2)
...
time_EA = time_EA[:-1]
Upper_Elastic_Energy = Upper_Elastic_Energy[:-1]
Lower_Elastic_Energy = Lower_Elastic_Energy[:-1]
Total_Energy = Total_Energy[:-1]
Kinetic_Energy = Kinetic_Energy[:-1]

upper_elastic_energy_append1 = np.array([Upper_Elastic_Energy[-1]]*5)
lower_elastic_energy_append1 = np.array([Lower_Elastic_Energy[-1]]*5)
total_energy_append1 = np.array([Total_Energy[-1]]*5)
kinetic_energy_append1 = np.array([Kinetic_Energy[-1]]*5)

time_EA = np.append(time_append, time_EA)
...
time_EA = np.append(time_EA, time_append1)
Upper_Elastic_Energy = np.append(Upper_Elastic_Energy, upper_elastic_energy_append1)
Lower_Elastic_Energy = np.append(Lower_Elastic_Energy, lower_elastic_energy_append1)
Total_Energy = np.append(Total_Energy, total_energy_append1)
Kinetic_Energy = np.append(Kinetic_Energy, kinetic_energy_append1)
...
ax3.plot(time, clutch2+2, 'r--', label='Up2')
ax3.plot(time, clutch4+6, 'g-', label='Low1')
ax3.plot(time, clutch3+4, 'k-.', label='Low2')
ax3.set_yticks([0, 1, 2, 3, 4, 5, 6, 7], ['OFF', 'ON', 'OFF', 'ON', 'OFF', 'ON', 'OFF', 'ON'], fontsize=12)
ax3.get_xaxis().set_visible(False)
ax3.legend(prop={'size': 10})

ax4 = plt.subplot(414, sharex=ax1)
plt.plot(time_EA, np.array(Kinetic_Energy)*1000, 'b-', label=r'$E_{kinetic}$')
plt.plot(time_EA, np.array(Upper_Elastic_Energy)*1000, 'r--', label=r'$E_{up}$')
plt.plot(time_EA, np.array(Lower_Elastic_Energy)*1000, 'c-.', label=r'$E_{low}$')
