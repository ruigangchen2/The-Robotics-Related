...
void Duty_50ms()  //for the key task
{
  ...
  case 2:  //If you want to send the matrix data, just click the key twice.
    ...
    Serial.print("clutch data\n");
    for(int i = 0; i < number_matrix; i++){  
      Serial.print(clutch_matrix[i]);
      Serial.print(",");
    }
    Serial.print("\nclutch data sending finished\n");
    Serial.print("\n");
...
void setup(){
  ...
  // Timer1.attachInterrupt(time_delay);
...
void speed_clutch_method(){
  ...
  if(speed_detect_state == 1){
    if(velocity < 30 && velocity > 0 && clutch_state == 0 && direction == 0 && m > 0){
      clutch_state = 1;
      direction = 1; //逆时针转
    }
    if(velocity > -30 && velocity < 0  && clutch_state == 0 && direction == 1 && m < 0){
      clutch_state = 1;
      direction = 0; //逆时针转
    }
  }
