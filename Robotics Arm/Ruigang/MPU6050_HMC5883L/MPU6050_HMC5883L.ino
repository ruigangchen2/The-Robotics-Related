...
void HMC5883L_Write(uint8_t register_address, uint8_t data)
{
 ...
 */
uint8_t HMC5883L_Read(uint8_t register_address, uint8_t buffer[], uint8_t length)
{
  ...
  Wire.requestFrom(HMC5883L, length);
...
void setup() {
  ...
  while(!I2Cdev::writeByte(MPU6050, 0x19, 7));  //  Set the sample rate to 1000Hz - 8kHz/(7+1) = 1000Hz
  while(!I2Cdev::writeByte(MPU6050, 0x1A, 0x00)); //  Disable FSYNC and set 260 Hz Acc filtering, 256 Hz Gyro filtering, 8 KHz sampling
  while(!I2Cdev::writeByte(MPU6050, 0x1B, 0x00)); //  Set Gyro Full Scale Range to ±250deg/s
  while(!I2Cdev::writeByte(MPU6050, 0x1C, 0x00)); //  Set Accelerometer Full Scale Range to ±2g

  I2Cdev::writeBit(MPU6050, 0x37, 1, 1); // set bypass mode for gateway to hmc5883L
  I2Cdev::writeBit(MPU6050, 0x6A, 5, 0); // set bypass mode for gateway to hmc5883L

  while(!I2Cdev::writeByte(MPU6050, 0x6B, 0x01)); // PLL with X axis gyroscope reference and disable sleep mode
  
  while(!I2Cdev::readBytes(MPU6050, 0x75, 1, i2cData));
...
void updateHMC5883L() {
  ...
  HMC5883L_Read(0x03,i2cData,6); // Get magnetometer values
