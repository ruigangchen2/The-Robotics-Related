...
gcc -o Robotics_Arm_Accelerate_Twice Robotics_Arm_Accelerate_Twice.c -lwiringPi -pthread -lcrypt -lm -lrt
...
float motion = 0; 
char clockwise_direction = '+';
char anticlockwise_direction = '-';
...
void get_info()  //calculate the information of the encoder
{
    ...
    if(current_direction == clockwise_direction){
        ...
        if(fds[0].revents&POLLPRI){
            ...
            if((len=read(fd,buffer,sizeof(buffer)))==-1){
                ...
                if(digitalRead(encoderB_pin) == HIGH){  //if B pin is in the high place
                    ...
                    current_direction = anticlockwise_direction;
                }
                pulse++;
                get_info();
...
void *Timer_5ms(void)
{
    ...
    int middle_state = 0;
    int Robotics_Arm_Clutch_State = 0;
    ...
    while(1){
        printf("\rThe current angle is: %.2f degrees, The current angular velocity is: %.2f degrees/s", motion, velocity);   
        fflush(stdout);   
        ...
        if(file_state == 1){
            ...
            if(motion < -18)Starting_State = 1;
            
            switch(Robotics_Arm_Clutch_State){
                case 0:
                    if(motion > -15 && Starting_State == 1){
                    ...
                    break;
                case 1:
                    if(abs(motion) - abs(previous_motion) > 10 && Starting_State == 1){
                        digitalWrite(electromagnet_1,1);
                        digitalWrite(electromagnet_2,0);
                        Electromagnet_Clutch_1 = 1;
                        Electromagnet_Clutch_2 = 0;
