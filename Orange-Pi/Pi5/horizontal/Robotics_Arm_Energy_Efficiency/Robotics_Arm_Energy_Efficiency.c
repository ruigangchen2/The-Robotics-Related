...
gcc -o Robotics_Arm_Energy_Efficiency Robotics_Arm_Energy_Efficiency.c -lwiringPi -pthread -lcrypt -lm -lrt
...
void speed_clutch_method(){
  if(premotion > motion && clutch_state == 0 && direction == 0 && speed_detect_state == 0){  
    speed_detect_state = 1;
    direction = 1;
  }

  if(speed_detect_state == 1){
    if(velocity < 20 && velocity > 0 && clutch_state == 0 && direction == 0){
      clutch_state = 1;
      direction = 1; 
    }
    if(velocity > -20 && velocity < 0  && clutch_state == 0 && direction == 1){
      clutch_state = 1;
      direction = 0;
    }
  }
    ...
    while(1){
        ...
        if(file_state == 1){
            ...
            if(clutch_state == 1){
                ...
                digitalWrite(electromagnet_1,1); // 100% PWM wave
